...
export function useRefused() {
  ...
  const handleRefreshList = useCallback(async () => {
    ...
    try {
      ...
      toast({
        type: "error",
        message: "Ocorreu um erro ao buscar por seus pedidos recusados",
      });
    } finally {
  ...
  const renderListItem = useCallback(
    ({ item }: ListRenderItemInfo<InventoryWithdrawRefused>) => (
      <WithdrawItem
        title={item.name}
        location={item.local}
  ...
  const keyExtractorList = useCallback(
    (item: InventoryWithdrawRefused) => item.id,
    []
  );
